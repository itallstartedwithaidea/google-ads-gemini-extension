 ...
 * in with via the Remote backend (ahmeego.com).
 *
 * Non-secret metadata (email, addedAt, accountsCount, active pointer)
 * lives in `sessions.json` next to this file. Secrets (sessionId, and
 * for v2.3-era entries an optional refreshToken) live in the OS keychain
 * via `keytar` when available; if keytar cannot load (e.g. Linux without
 * libsecret), we fall back to `sessions.secrets.json` with 0600 file
 * permissions. Both files are listed in .gitignore.
 *
 * v2.4 note: new sign-ins no longer store a refreshToken — the Google
 * OAuth refresh token stays encrypted on ahmeego.com. v2.3 users
 * auto-upgrade: existing refreshTokens continue to work for auto-refresh,
 * and the next `save()` for that identity silently drops them from
...
export async function save({ email, sessionId, refreshToken = null, accountsCount }) {
  if (!email) throw new Error("session-store.save: email is required");
  if (!sessionId) throw new Error("session-store.save: sessionId is required");
  // v2.4+: refreshToken is optional. Writing a fresh session deliberately
  // omits it so v2.3 users who re-login auto-migrate to a slimmer record.
  const secretPayload = refreshToken
    ? { refreshToken, sessionId }
    : { sessionId };
  const backend = await writeSecret(email, secretPayload);
